...
void to_json(json &json, Box &Box) {
     j = json {
          { "vec1", {
               { "x", Box.x1},
               { "y", Box.y1}
...
void recogniseStickersByThreshold(Mat &image,vector<vector<Point>> &Line1, vector<vector<Point>> &Line2) {
    ...
    if (Line1[i].size()>size){
        ...
        for (int j=0;j<size;j++)
          ...
          if (Line1[0].size()>size) {
               Box box;
               box.x1 = Line1[0].back().x;
               box.y1 = Line1[0].back().y;
               box.x2 = Line2[0].back().x;
               box.y2 = Line2[0].back().y;
               json json;
               to_json(json,box);
               stringstream ss;
               string str;
